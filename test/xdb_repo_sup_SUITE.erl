...
%% Test Cases
-export([t_start_stop/1, t_return_ignore/1, t_missing_repo_config/1, t_no_repo_init_fun/1,
         t_sup_spec/1]).
...
t_start_stop(_Config) ->
    {ok, Pid} = xdb_repo_sup:start_link(xdb_test_repo, cross_db, ?ADAPTER, []),
    ok = xdb_repo_sup:stop(wrong),
    true = process_info(Pid) /= undefined,
    ok = xdb_repo_sup:stop(xdb_test_repo),
    _ = timer:sleep(500),
...
t_return_ignore(_Config) ->
    ignore = xdb_repo_sup:start_link(xdb_test_repo, cross_db, ?ADAPTER, [{start, ignore}]),
    ok.
...
t_missing_repo_config(_Config) ->
    ...
    {error, {Expected, _}} =
        assert_error(fun() -> xdb_repo_sup:start_link(other_repo, cross_db, ?ADAPTER, []) end,
                     badarg),
    ok.

-spec t_no_repo_init_fun(xdb_ct:config()) -> ok.
t_no_repo_init_fun(_Config) ->
    _ = xdb_repo_sup:start_link(xdb_test_repo_and_adapter,
                                cross_db,
                                xdb_test_repo_and_adapter,
                                []),
    ok.
...
t_sup_spec(_Config) ->
    #{id := xdb_test_repo,
      type := supervisor,
      start := {xdb_test_repo, start_link, []}} =
        xdb_test_repo:supervisor(),
    ok.
