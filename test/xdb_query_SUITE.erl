...
-export([all/0]).
%% Test Cases
-export([t_from/1, t_validate_operator/1, t_pk_filter/1, t_invalid_query/1]).
...
t_from(_Config) ->
    ...
    #{source := people, from := person} = xdb_query:from(person),
    ok.

-spec t_validate_operator(xdb_ct:config()) -> ok.
t_validate_operator(_Config) ->
    ok =
        lists:foreach(fun(Op) -> Op = xdb_query:validate_operator(Op) end, xdb_query:operators()),

    ok =
        assert_error(fun() -> xdb_query:validate_operator(invalid) end,
                     {unknown_operator, invalid}).
...
t_invalid_query(_Config) ->
    ok =
        assert_error(fun() -> xdb_query:from(person, [{where, {id, 10}}]) end,
                     {invalid_query_error, "invalid query entry: {where,{id,10}}"}).
