...
-export([all/0]).
%% Test Cases
-export([t_to_bin/1, t_to_atom/1, t_to_kvlist/1, t_to_map/1, t_is_datetime/1,
         t_stringify/1, t_keyfetch/1, t_keyfind/1, t_keymerge/1, t_kv_keys_values/1,
         t_reduce_while/1, t_raise/1]).
...
t_keyfetch(_Config) ->
    ...
    ok =
        xdb_ct:assert_error(fun() -> xdb_lib:keyfetch(bar, [{foo, bar}, {a, 1}]) end,
                            {badkey, bar}).
...
t_reduce_while(_Config) ->
    ...
    try
        xdb_lib:reduce_while(fun(V, Acc) -> a = V ++ Acc end, 0, lists:seq(1, 10))
    catch
        _:_ ->
            ok
    end.
...
t_raise(_Config) ->
    ok = xdb_ct:assert_error(fun() -> xdb_lib:raise(my_error) end, my_error),

    ok =
        xdb_ct:assert_error(fun() -> xdb_lib:raise(my_error, "my reason") end,
                            {my_error, "my reason"}),

    ok =
        xdb_ct:assert_error(fun() -> xdb_lib:raise(my_error, "missing ~p", [{a, 1}]) end,
                            {my_error, "missing {a,1}"}).
