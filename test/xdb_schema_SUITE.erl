...
%% Test Cases
-export([t_schema/1, t_get_set_fields/1]).
...
t_schema(_Config) ->
    ...
    ok =
        assert_error(fun() -> xdb_schema:get_pk_values([id], Schema) end,
                     no_primary_key_value_error),

    NormalizedFields =
        xdb_schema:normalize_keys(#{first_name => <<"John">>, <<"last_name">> => <<"Doe">>}),
    #{first_name := <<"John">>, last_name := <<"Doe">>} = NormalizedFields,
    ok.
...
t_get_set_fields(_Config) ->
    ...
    Schema1 =
        xdb_schema:set_fields(Schema,
                              #{first_name => <<"other">>, <<"last_name">> => <<"other">>}),

    #{first_name := <<"other">>, last_name := <<"other">>} = xdb_schema:fields(Schema1),
    ...
    ok =
        xdb_ct:assert_error(fun() -> xdb_schema:set_field(Schema, id, <<"hello">>) end,
                            {bad_type, {id, integer, <<"hello">>}}).
