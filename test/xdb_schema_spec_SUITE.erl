...
%% Test Cases
-export([t_schema/1, t_getters/1, t_setters/1, t_disabled_getters_setters/1]).
...
t_schema(Config) ->
    ...
    FieldNames =
        lists:usort(
            xdb_schema_spec:field_names(Schema)),
...
t_setters(_Config) ->
    ...
    33 =
        person:age(
            person:age(Person, 33)),
    <<"Other">> =
        person:last_name(
            person:last_name(Person, <<"Other">>)),
    ok.
...
t_disabled_getters_setters(_Config) ->
    ...
    ok = xdb_ct:assert_error(fun() -> Mod:profile_image(Person) end, undef),

    ok =
        xdb_ct:assert_error(fun() -> Mod:created_at(Person, calendar:universal_time()) end,
                            undef).
