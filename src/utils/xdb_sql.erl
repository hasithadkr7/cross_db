...
%% Basic SQL builder
-export([i/2, i_all/2, u/3, d/1, d/2, s/7, s_count/3, s_count/4]).
%% Query utilities
-export([parse_conditions/1, where_clause/1, where_clause/2, where_clause/3,
         order_by_clause/1, order_by_clause/2]).
%% Extras
-export([escape/1, slot_numbered/1, slot_question/1]).
...
-spec s_count(Table, SelectFields, ExtraWhere) -> Res
    when Table :: tab_name(),
         SelectFields :: [field()],
         ExtraWhere :: string(),
         Res :: {iolist(), [term()]}.
...
%% @doc Returns number of results, useful for pagination.
-spec s_count(Table, SelectFields, Conditions, ExtraWhere) -> Res
    when Table :: tab_name(),
         SelectFields :: [field()],
...
s_count(Table, SelectFields, Conditions, ExtraWhere) ->
    ...
    {["SELECT COUNT(1) AS `count` FROM ", escape(Table), " WHERE ", Where], Values}.

%% @doc Generic select function.
-spec s(Table, SelectFields, Conditions, ExtraWhere, Page, PageSize, OrderBy) -> Res
    when Table :: tab_name(),
         SelectFields :: [field()],
         Conditions :: condition(),
         ExtraWhere :: string(),
...
s(Table, SelectFields, Conditions, ExtraWhere, Page, PageSize, OrderBy) ->
    Paging =
        case PageSize of
            0 ->
                [];
            _ ->
                [" LIMIT ", integer_to_list(Page), ", ", integer_to_list(PageSize)]
        end,
    ...
    case Where of
        [] ->
            {["SELECT ", Select, " FROM ", escape(Table), " ", OrderBy, " ", Paging], Values};
        _ ->
            {["SELECT ",
              Select,
              " FROM ",
...
%% @doc INSERT.
-spec i(Table, PropList) -> Res
    when Table :: tab_name(),
         PropList :: proplists:proplist(),
         Res :: {iodata(), [term()]}.
i(Table, PropList) ->
    {Fields, Values, Args} =
        lists:foldr(fun({K, V}, {Fs, Vs, Args}) -> {[escape(K) | Fs], [V | Vs], ["?" | Args]} end,
                    {[], [], []},
                    PropList),
...
-spec i_all(SchemaMeta, PropList) -> Res
    when SchemaMeta :: xdb_adapter:schema_meta(),
         PropList :: proplists:proplist(),
         Res :: {iodata(), [term()]}.
...
%% @doc UPDATE.
-spec u(Table, UpdateFields, Conditions) -> Res
    when Table :: tab_name(),
...
u(Table, UpdateFields, Conditions) ->
    ...
    {UFields, UValues} =
        lists:foldr(fun({K, V}, {Fs, Vs}) -> {[escape(K) ++ "=?" | Fs], [V | Vs]} end,
                    {[], []},
                    UpdateFields),
    ...
    case Where of
        [] ->
            {["UPDATE ", escape(Table), " SET ", Update], UValues, WValues};
        _ ->
            {["UPDATE ", escape(Table), " SET ", Update, " WHERE ", Where], UValues, WValues}
    end.
...
-spec parse_conditions(Expr) -> Res
    when Expr :: xdb_query:conditions(),
...
parse_conditions({Op, Exprs}, {Values, CleanExprs, Count}) when ?logical_op(Op) ->
    ...
    {NewValues, [{Op, lists:reverse(NewCleanExprs)} | CleanExprs], NewCount};
...
parse_conditions({Name, Op, Value}, {Values, CleanExprs, Count})
...
parse_conditions({Name1, Op, Name2}, {Values, CleanExprs, Count}) when is_atom(Name2) ->
    {Values, [{Name1, xdb_query:validate_operator(Op), Name2} | CleanExprs], Count};
...
order_by_clause(SortFields, EscapeFun) ->
    ClauseFun =
        fun({Name, SortOrder}) -> [EscapeFun(atom_to_list(Name)), " ", atom_to_list(SortOrder)]
        end,
...
form_insert_query(Schema, [PropList0 | Remaining], {_F0, V0, A0}) ->
    ...
    PropList =
        maps:to_list(
            maps:merge(
                maps:from_list([{K, null} || K <- FieldNames]), PropList0)),
    {Fields, Values, Args} =
        lists:foldr(fun({K, V}, {Fs, Vs, Args}) -> {[escape(K) | Fs], [V | Vs], ["?" | Args]} end,
                    {[], [], []},
                    PropList),
