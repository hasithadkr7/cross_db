...
-export([to_bin/1, to_atom/1, to_kvlist/1, to_map/1, is_datetime/1, stringify/2,
         keyfetch/2, keyfind/2, keyfind/3, keymerge/2, kv_keys/1, kv_values/1, reduce_while/3,
...
-export_type([kwentry/0, keyword/0, kw_map/0, kv/0, kvlist/0]).
...
keymerge(KV1, KV2) ->
    lists:foldl(fun({K, V}, Acc) -> lists:keystore(K, 1, Acc, {K, V}) end, KV1, KV2).
...
-spec reduce_while(Fun, AccIn, List) -> Result
    when Fun :: fun((Elem :: any(), Acc :: any()) -> FunRes),
