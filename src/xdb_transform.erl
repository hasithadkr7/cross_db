...
parse_transform(ASTs, _Options) ->
    try
        Result =
            lists:flatten([erl_syntax_lib:map(fun(T) -> transform(erl_syntax:revert(T)) end, AST)
                           || AST <- ASTs]),
        ...
        error:{badadapter, Adapter} ->
            Error =
                "adapter ~p was not compiled, ensure it is correct "
...
schema_spec(Name, [{FName, FType, FOpts} | Fields]) ->
    ...
    FunBody =
        lists:foldl(fun ({FName1, FType1, FOpts1}, Acc) ->
    ...
    add_funs(lists:flatten([{schema_spec, 0, ?Q(FunBody)}, schema(), primary_key()])).
...
type_spec([{FName, FType, _} | Fields]) ->
    ...
    Type =
        lists:foldl(fun ({FName1, FType1, _}, Acc) ->
...
build_repo(Opts) ->
    ...
    RepoFuns =
        [repo_fun_template(M, F, Arity, Repo, Adapter) || {M, F, Arity} <- repo_api_specs()],
...
%% @private
repo_api_specs() ->
    [{xdb_repo_schema, insert, 1}, {xdb_repo_schema, insert, 2},
     {xdb_repo_schema, insert_or_raise, 1}, {xdb_repo_schema, insert_or_raise, 2},
     {xdb_repo_schema, update, 1}, {xdb_repo_schema, update, 2},
     {xdb_repo_schema, update_or_raise, 1}, {xdb_repo_schema, update_or_raise, 2},
     {xdb_repo_schema, delete, 1}, {xdb_repo_schema, delete, 2},
     {xdb_repo_schema, delete_or_raise, 1}, {xdb_repo_schema, delete_or_raise, 2},
     {xdb_repo_queryable, all, 1}, {xdb_repo_queryable, all, 2},
     {xdb_repo_queryable, all_by, 2}, {xdb_repo_queryable, get, 2},
     {xdb_repo_queryable, get, 3}, {xdb_repo_queryable, get_or_raise, 2},
     {xdb_repo_queryable, get_or_raise, 3}, {xdb_repo_queryable, get_by, 2},
     {xdb_repo_queryable, get_by, 3}, {xdb_repo_queryable, get_by_or_raise, 2},
     {xdb_repo_queryable, get_by_or_raise, 3}, {xdb_repo_schema, insert_all, 2},
     {xdb_repo_schema, insert_all, 3}, {xdb_repo_queryable, delete_all, 1},
     {xdb_repo_queryable, delete_all, 2}, {xdb_repo_queryable, update_all, 2},
...
maybe_add_funs(Result) ->
    case erlang:get(funs) of
        undefined ->
            Result;
        ExtFuns ->
            lists:droplast(Result) ++ ExtFuns ++ [lists:last(Result)]
    end.
...
build_export([{FirstFun, FirstArity} | Exports]) ->
    ?Q(lists:foldl(fun({Fun, Arity}, Acc) -> Acc ++ text(", ~p/~p", [Fun, Arity]) end,
                   text("-export([~p/~p", [FirstFun, FirstArity]),
                   Exports)
       ++ "]).");
build_export(_) ->
    ?Q("-export([]).").
...
%% @private
add_funs(FunSpecs) ->
    lists:foreach(fun({Name, Arity, Body}) -> add_fun(Name, Arity, Body) end, FunSpecs).
...
do_splicing_args([H | T]) ->
    lists:foldl(fun(N, Acc) -> Acc ++ ", A" ++ integer_to_list(N) end,
                "A" ++ integer_to_list(H),
                T).
