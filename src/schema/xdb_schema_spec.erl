...
%% API
-export([new/2, new_field/2, new_field/3, name/1, fields/1, field_names/1, field_types/1,
         pk_fields/1, pk_field_names/1]).
...
%% Field Types
-type field_type() ::
    string | integer | float | boolean | date | datetime | binary | custom.
%% Common field options
-type field_opt() :: primary_key.
-type field_opts() :: [field_opt() | any()].
%% Field spec
-type field() :: {Name :: atom(), Type :: field_type(), Opts :: [field_opt() | any()]}.
%% Schema fields definition
-type fields() :: [field()].
%% Schema spec
-type t() :: #{name := atom(), fields := fields()}.

%% Exported types
-export_type([field_type/0, field/0, fields/0, t/0]).
...
field_types(#{fields := Fields}) ->
    lists:foldl(fun({FieldName, FieldType, _}, Acc) -> Acc#{FieldName => FieldType} end,
                #{},
                Fields).
