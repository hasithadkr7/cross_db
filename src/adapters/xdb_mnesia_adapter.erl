...
-export([transaction/3, in_transaction/1, rollback/2]).
...
do_insert_all(Source, PKs, FieldNames, List, OnConflict) ->
    ...
    InsertAll = maybe_transaction(fun() -> xdb_lib:reduce_while(ReduceFun, [], List) end),
...
do_execute(all, Source, PKs, FieldNames, {pk_query, PkValues}) ->
    GetTx = fun() -> mnesia:read({Source, pk(xdb_lib:kv_values(PkValues))}) end,
...
do_execute(all, Source, PKs, FieldNames, {MatchSpec, Query}) ->
    ...
    SelectAll = fun() -> mnesia:select(Source, MatchSpec) end,
...
do_execute(update_all, Source, PKs, FieldNames, {MatchSpec, Updates}) ->
    ...
    Transaction =
        fun() ->
           [execute_update(Record, Source, PKs, FieldNames, Fields)
            || Record <- mnesia:select(Source, MatchSpec)]
        end,
