...
%% Properties
-export([schema/1, data/1, params/1, errors/1, changes/1, is_valid/1, types/1, required/1,
         repo/1, action/1, filters/1]).
%% API
-export([add_error/3, add_error/4, apply_changes/1, cast/3, change/2, get_field/2,
         get_field/3, put_change/3, get_change/2, get_change/3, delete_change/2, validate_change/3,
         validate_required/2, validate_inclusion/3, validate_number/3, validate_integer/2,
         validate_length/3, validate_format/3]).
...
-spec cast(Data, Params, Permitted) -> Res
    when Data :: xdb_schema:t() | t(),
...
do_cast({SchemaName, Data, Types}, Params, Permitted) ->
    ...
    {Changes, Errors, IsValid} =
        maps:fold(fun(ParamKey, ParamVal, Acc) -> process_param(ParamKey, ParamVal, Types, Acc)
                  end,
                  {#{}, [], true},
                  FilteredParams),

    (changeset())#{schema := SchemaName,
                   data := Data,
...
get_changed(Changeset, NewChanges) ->
    maps:fold(fun(K, V, Acc) -> put_change(Acc, K, V) end, Changeset, NewChanges).
...
validate_change(#{changes := Changes, errors := Errors} = Changeset, Field, Validator) ->
    ...
    NewErrors2 = [validate_change_error(Error) || Error <- NewErrors1],

    case NewErrors2 of
        [] ->
            Changeset;
        [_ | _] ->
            Changeset#{errors := NewErrors2 ++ Errors, is_valid := false}
    end.
...
validate_number(Changeset, Field, Opts) ->
    validate_change(Changeset,
                    ...
                    fun(TargetField, Value) ->
                       hd([validate_number_change(SpecKey, TargetValue, TargetField, Value)
                           || {SpecKey, TargetValue} <- Opts])
                    end).

%% @private
validate_number_change(SpecKey, TargetValue, TargetField, Value) ->
...
validate_number(Field, Value, Message, SpecFun, TargetValue) ->
    case SpecFun(Value, TargetValue) of
        true ->
            [];
        false ->
            [{Field, {Message, [{validation, number}]}}]
    end.
...
validate_integer(Changeset, Key) ->
    case is_integer(get_field(Changeset, Key)) of
        true ->
            Changeset;
        false ->
            add_error(Changeset, Key, <<"must be integer">>)
    end.
...
validate_length(Changeset, Field, Opts) ->
    validate_change(Changeset,
                    Field,
                    fun(_, Value) ->
                       case do_validate_length(length_validators(),
                                               Opts,
                                               byte_size(Value),
                                               undefined)
                       of
                           undefined ->
                               [];
                           Message ->
                               [{Field, {Message, [{validation, length}]}}]
                       end
                    end).
...
process_param(ParamKey, ParamValue, Types, {Changes, Errors, IsValid}) ->
    ...
    case cast_field(Key, ParamValue, Types) of
        ...
        {invalid, Type} ->
            {Changes,
             [{Key, {<<"is invalid">>, [{type, Type}, {validation, cast}]}} | Errors],
             false};
        missing ->
            {Changes, Errors, IsValid}
    end.
