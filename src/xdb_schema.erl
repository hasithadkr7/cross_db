...
%% API
-export([new/2, module/1, source/1, metadata/1, fields/1, pk_fields/1, get_pk_values/2,
         set_fields/2, set_field/3, get_fields/2, get_field/2, normalize_keys/1]).
...
-type fields() :: #{atom() | binary() => any()}.
%% Schema definition
-type t() :: #{'__meta__' := metadata(), atom() => any() | undefined}.

%% Exported types
-export_type([metadata/0, fields/0, t/0]).
...
pk_fields(#{'__meta__' := #{schema := Mod}} = Schema) ->
    PkFieldNames =
        xdb_schema_spec:pk_field_names(
            Mod:schema_spec()),
...
get_pk_values(PKs, Fields) ->
    [get_pk_fetch_key(PK, Fields) || PK <- PKs].

%% @private
get_pk_fetch_key(PK, Fields) ->
    try
        fetch_key(PK, Fields)
    catch
        error:{badkey, K} ->
...
normalize_keys(Fields) when is_map(Fields) ->
    maps:fold(fun (K, V, Acc) when is_binary(K) ->
                      Acc#{cast_field_name(K) => V};
                  (K, _, Acc) when is_atom(K) ->
                      Acc
              end,
              Fields,
              Fields).
...
init_schema(Module) ->
    ...
    lists:foldl(fun(FieldName, Acc) -> Acc#{FieldName => undefined} end,
                #{'__meta__' => Meta},
                FieldNames).
...
cast_field(Type, Name, Value) ->
    case xdb_schema_type:cast(Type, Value) of
        {ok, NewValue} ->
            NewValue;
        {error, _} ->
            error({bad_type, {Name, Type, Value}})
    end.
