...
-export([new/2, new/3, new/4, changeset/2, all/2, list_to_map/1]).
...
-spec new(binary(), binary(), integer() | undefined, binary() | undefined) ->
             xdb_schema:fields().
...
all(Repo, Conditions) ->
    Records =
        list_to_map(Repo:all(
                        xdb_query:from(person, [{where, Conditions}]))),
    {maps:size(Records), Records}.

-spec list_to_map([t()]) -> #{any() => t()}.
list_to_map(People) ->
    lists:foldl(fun(#{id := Id} = P, Acc) -> Acc#{Id => P} end, #{}, People).
