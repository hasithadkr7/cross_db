...
%% Test Cases
-export([t_transaction/1, t_transaction_error/1, t_in_transaction/1, t_rollback/1]).
...
-spec all() -> [atom()].
all() ->
    [t_transaction, t_transaction_error, t_in_transaction, t_rollback].
...
t_transaction(Config) ->
    ...
    {ok, [_, _]} =
        Repo:transaction(fun() ->
                            ...
                            {ok, #{id := 10}} =
                                Repo:insert(
                                    person:schema(#{id => 10, first_name => "Other"})),
...
t_rollback(Config) ->
    ...
    ok = assert_error(fun() -> Repo:rollback(my_error) end, no_transaction_is_active).
