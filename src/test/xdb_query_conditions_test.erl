...
%% Test Cases
-export([t_all/1, t_or_conditional/1, t_and_conditional/1, t_not_conditional/1,
         t_not_null_conditional/1, t_null_conditional/1, t_operators/1, t_like_operator/1,
         t_deeply_nested/1]).
...
t_or_conditional(Config) ->
    ...
    Conditions1 =
        [{'or', [{first_name, <<"John">>}, {first_name, <<"Joe">>}, {first_name, <<"Alan">>}]}],
    {3,
     #{1 := #{first_name := <<"Alan">>, last_name := <<"Turing">>},
       3 := #{first_name := <<"Alan">>, last_name := <<"Poe">>},
    ...
    Conditions2 =
        [{first_name, <<"Alan">>}, {'or', [{last_name, <<"Poe">>}, {last_name, <<"Lennon">>}]}],
    {1, #{3 := #{first_name := <<"Alan">>, last_name := <<"Poe">>}}} =
        person:all(Repo, Conditions2),

    Conditions3 = [{'or', [{age, '=<', 40}, {birthdate, '<', {1900, 1, 1}}]}],
    {3,
...
t_and_conditional(Config) ->
    ...
    Conditions1 =
        [{'and', [{first_name, <<"John">>}, {last_name, <<"Lennon">>}, {age, '>', 50}]}],
    [] =
        Repo:all(
            xdb_query:from(person, [{where, Conditions1}])),

    Conditions2 =
        [{'and', [{first_name, <<"John">>}, {last_name, <<"Lennon">>}, {age, '==', 40}]}],
    {1, #{4 := #{first_name := <<"John">>, last_name := <<"Lennon">>}}} =
        person:all(Repo, Conditions2),

    Conditions3 =
        [{'and',
          [{first_name, <<"Alan">>}, {'or', [{last_name, <<"Poe">>}, {last_name, <<"Turing">>}]}]}],
    {2,
     #{1 := #{first_name := <<"Alan">>, last_name := <<"Turing">>},
       3 := #{first_name := <<"Alan">>, last_name := <<"Poe">>}}} =
        person:all(Repo, Conditions3),
    ...
    ok =
        assert_error(fun() -> xdb_query:validate_operator('<>') end, {unknown_operator, '<>'}).

-spec t_not_conditional(xdb_ct:config()) -> ok.
t_not_conditional(Config) ->
    Repo = xdb_lib:keyfetch(repo, Config),

    Conditions1 = [{'not', {first_name, <<"Alan">>}}],
    {2,
     #{2 := #{first_name := <<"Charles">>, last_name := <<"Darwin">>},
       4 := #{first_name := <<"John">>, last_name := <<"Lennon">>}}} =
        person:all(Repo, Conditions1),

    Conditions2 = [{'not', {'or', [{age, '=<', 40}, {birthdate, '<', {1900, 1, 1}}]}}],
    {1, #{1 := #{first_name := <<"Alan">>, last_name := <<"Turing">>}}} =
        person:all(Repo, Conditions2),
    ok.
...
t_operators(Config) ->
    ...
    Conditions1 = [{'and', [{age, not_null}, {age, '<', 50}]}],
    {3,
     #{1 := #{first_name := <<"Alan">>, last_name := <<"Turing">>},
       3 := #{first_name := <<"Alan">>, last_name := <<"Poe">>},
       4 := #{first_name := <<"John">>, last_name := <<"Lennon">>}}} =
    ...
    Conditions2 = [{'and', [{age, not_null}, {age, '>', 50}]}],
    {1, #{2 := #{first_name := <<"Charles">>, last_name := <<"Darwin">>}}} =
        person:all(Repo, Conditions2),

    Conditions3 = [{'and', [{age, not_null}, {age, '>', 100}]}],
    [] =
        Repo:all(
            xdb_query:from(person, [{where, Conditions3}])),

    Conditions4 = [{'and', [{age, not_null}, {age, '>=', 41}]}],
    {2,
     #{1 := #{first_name := <<"Alan">>, last_name := <<"Turing">>},
       2 := #{first_name := <<"Charles">>, last_name := <<"Darwin">>}}} =
        person:all(Repo, Conditions4),

    Conditions5 = [{'and', [{age, not_null}, {age, '=<', 40}]}],
    {2,
     #{3 := #{first_name := <<"Alan">>, last_name := <<"Poe">>},
       4 := #{first_name := <<"John">>, last_name := <<"Lennon">>}}} =
        person:all(Repo, Conditions5),

    Conditions6 = [{'and', [{age, not_null}, {age, '==', 41}]}],
    {1, #{1 := #{first_name := <<"Alan">>, last_name := <<"Turing">>}}} =
        person:all(Repo, Conditions6),

    Conditions7 = [{'and', [{age, not_null}, {age, '/=', 40}]}],
    {2,
     #{1 := #{first_name := <<"Alan">>, last_name := <<"Turing">>},
       2 := #{first_name := <<"Charles">>, last_name := <<"Darwin">>}}} =
...
t_deeply_nested(Config) ->
    ...
    Conditions1 =
        [{address, not_null},
         {'or',
          [{'and', [{age, '>=', 40}, {birthdate, '<', {1900, 1, 1}}]}, {address, <<"abc">>}]}],
    {3,
     #{1 := #{first_name := <<"Alan">>, last_name := <<"Turing">>},
       2 := #{first_name := <<"Charles">>, last_name := <<"Darwin">>},
       3 := #{first_name := <<"Alan">>, last_name := <<"Poe">>}}} =
    ...
    Conditions2 =
        [{'or',
          [{'and', [{age, '>', 40}, {'or', [{address, <<"abc">>}, {address, <<"cde">>}]}]},
           {first_name, <<"John">>}]}],
    {2,
     #{1 := #{first_name := <<"Alan">>, last_name := <<"Turing">>},
       4 := #{first_name := <<"John">>, last_name := <<"Lennon">>}}} =
