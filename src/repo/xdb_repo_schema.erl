...
%% API
-export([insert_all/4, insert_all/5, insert/3, insert/4, insert_or_raise/3,
         insert_or_raise/4, update/3, update/4, update_or_raise/3, update_or_raise/4, delete/3,
         delete/4, delete_or_raise/3, delete_or_raise/4]).
...
-spec insert_all(Repo, Adapter, Schema, Entries, Opts) -> Res
...
-spec insert(Repo, Adapter, Data, Opts) -> Res
    when Repo :: xdb_repo:t(),
         Adapter :: xdb_adapter:t(),
...
-spec insert_or_raise(Repo, Adapter, Data, Opts) -> Res
    when Repo :: xdb_repo:t(),
         Adapter :: xdb_adapter:t(),
         Data :: xdb_schema:t() | xdb_changeset:t(),
         Opts :: xdb_lib:keyword(),
...
-spec update(Repo, Adapter, Changeset, Opts) -> Res
    when Repo :: xdb_repo:t(),
         Adapter :: xdb_adapter:t(),
...
-spec update_or_raise(Repo, Adapter, Changeset, Opts) -> Res
    when Repo :: xdb_repo:t(),
         Adapter :: xdb_adapter:t(),
         Changeset :: xdb_changeset:t(),
         Opts :: xdb_lib:keyword(),
...
-spec delete(Repo, Adapter, Data, Opts) -> Res
    when Repo :: xdb_repo:t(),
         Adapter :: xdb_adapter:t(),
         Data :: xdb_schema:t() | xdb_changeset:t(),
         Opts :: xdb_lib:keyword(),
...
-spec delete_or_raise(Repo, Adapter, Data, Opts) -> Res
    when Repo :: xdb_repo:t(),
         Adapter :: xdb_adapter:t(),
         Data :: xdb_schema:t() | xdb_changeset:t(),
         Opts :: xdb_lib:keyword(),
...
%% @private
put_repo_and_action(#{action := Given}, Action, Repo)
    when Given /= undefined, Given /= Action ->
...
load_changes(Changeset, State, Values) ->
    try
        ...
        error:{bad_type, {FName, FType, FValue}} ->
            ...
            Reason =
                xdb_lib:stringify(Text, [FValue, FType, FName, xdb_changeset:schema(Changeset)]),
            error({badarg, Reason})
    end.
...
add_pk_filter(Schema, Filters) ->
    maps:fold(fun(K, V, Acc) -> [{K, V} | Acc] end,
              xdb_lib:to_kvlist(Filters),
              xdb_schema:pk_fields(Schema)).
...
postprocess({error, stale}, Changeset) ->
    xdb_exception:stale_entry_error(
        xdb_changeset:action(Changeset), xdb_changeset:data(Changeset));
